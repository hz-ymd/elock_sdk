...
public class CircleFlowIndicator extends View implements FlowIndicator, AnimationListener {
	...
	private float radius = 8;//4;
	private float circleSeparation = 2 * radius + radius;
	private float activeRadius = 2.0f;//0.5f;
...
//		initColors(0xff0000, 0x1b82d2, STYLE_FILL, STYLE_STROKE);
...
//		int activeDefaultColor = 0xFF2c404e;//0xffff0000;
		int activeDefaultColor = 0xFFFFFFFF;//0xffff0000;
...
//		int inactiveDefaultColor = 0x442c404e;//0xff1b82d2;//0x44FFFFFF;
		int inactiveDefaultColor = 0x44FFFFFF;//0x44FFFFFF;
		// Get a custom inactive color if there is one
		int inactiveColor = a.getColor(R.styleable.CircleFlowIndicator_inactiveColor, inactiveDefaultColor);

		// Retrieve the radius
//		radius = a.getDimension(R.styleable.CircleFlowIndicator_viewFlowRadius, 4.0f);
		radius = a.getDimension(R.styleable.CircleFlowIndicator_radius, 8.0f);

		circleSeparation = a.getDimension(R.styleable.CircleFlowIndicator_circleSeparation, 2 * radius + radius +10); //小圆圈的间隔
		activeRadius = a.getDimension(R.styleable.CircleFlowIndicator_activeRadius, 2.0f);//0.5f
		// Retrieve the fade out time
		fadeOutTime = a.getInt(R.styleable.CircleFlowIndicator_fadeOut, 0);
		fadeOutTime = 0; //如果不为0，广告页的小圆圈会消失再显示，再消失
//		mCentered = a.getBoolean(R.styleable.CircleFlowIndicator_viewFlowCentered, false);
...
//		canvas.drawCircle(leftPadding + radius + cx + centeringOffset, getPaddingTop() + radius, radius + activeRadius, mPaintActive);
		canvas.drawCircle(leftPadding + radius + cx + centeringOffset, getPaddingTop() + radius, radius + 1, mPaintActive);
